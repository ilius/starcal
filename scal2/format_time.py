...
def compileTmFormat(format, hasTime=True):
    ...
    while i<n:
        ...
        if c1=='Y':
            ...
            funcs.append(lambda cell, mode, tm: _(core.modules[mode].name))
            pyFmt += '%s'
            i += 2
            continue
        elif c1=='a':
            ...
            funcs.append(lambda cell, mode, tm: _(core.modules[mode].getMonthNameAb(cell.dates[mode][1])))
            pyFmt += '%s'
            i += 2
            continue
        elif c1=='B':
            funcs.append(lambda cell, mode, tm: _(core.modules[mode].getMonthName(cell.dates[mode][1])))
            pyFmt += '%s'
            i += 2
            continue
