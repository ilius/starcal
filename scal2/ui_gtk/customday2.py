...
class CustomDayDialog(gtk.Window):
    def __init__(self, main, month=None, day=None):
        ...
        self.set_icon_from_file(ui.logo)
        ...
        for m in core.modules[core.primaryMode].monthName:
            combo3.append_text(_(m))
        combo3.set_active(0)
        ...
        self.tboxDLen = TimeBox(hms=time.localtime()[3:6], lang=core.langSh)
        ...
        self.dboxStart = DateBox(lang=core.langSh)
        hbox.pack_start(self.dboxStart, 0, 0)
        hbox.pack_start(gtk.Label(''), 1, 1)
        hbox.pack_start(gtk.Label(_('Time')), 0, 0)
        self.tboxStart = TimeBox(hms=time.localtime()[3:6], lang=core.langSh)
        ...
        self.dboxEnd = DateBox(lang=core.langSh)
        hbox.pack_start(self.dboxEnd, 0, 0)
        hbox.pack_start(gtk.Label(''), 1, 1)
        hbox.pack_start(gtk.Label(_('Time')), 0, 0)
        self.tboxEnd = TimeBox(hms=time.localtime()[3:6], lang=core.langSh)
        ...
        self.tbox = TimeBox(hms=time.localtime()[3:6], lang=core.langSh)
        ...
        if ui.autoLocale:
            ...
            self.but_add.set_image(gtk.image_new_from_stock(gtk.STOCK_ADD,gtk.ICON_SIZE_BUTTON))
            ##########
            self.but_edit.set_label(_('_Edit'))
            self.but_edit.set_use_underline(True)
            self.but_edit.set_image(gtk.image_new_from_stock(gtk.STOCK_EDIT,gtk.ICON_SIZE_BUTTON))
            ##########
            self.but_del.set_label(_('_Delete'))
            self.but_del.set_use_underline(True)
            self.but_del.set_image(gtk.image_new_from_stock(gtk.STOCK_DELETE,gtk.ICON_SIZE_BUTTON))
            #########
            self.but_ok.set_label(_('_OK'))
            self.but_ok.set_use_underline(True)
            self.but_ok.set_image(gtk.image_new_from_stock(gtk.STOCK_OK,gtk.ICON_SIZE_BUTTON))
        butBox.add(self.but_add)
        butBox.add(self.but_edit)
        butBox.add(self.but_del)
        winVbox.pack_start(butBox, 0, 0)
        #############################################
        obox = CustomEventEditorWidget(self)
    ...
    def dateModeChanged(self, widget):
        for i in range(12):
            self.comboMonth.remove_text(0)
        for m in core.modules[core.primaryMode].monthName:
            self.comboMonth.append_text(_(m))
        self.comboMonth.set_active(0)
    ...
    def load_list(self):
        ...
        for record in self.db:
            ...
            try:
                ...
                p=None
            try:
                self.treestore.append([p, lst[0]+'/'+lst[1], lst[3], element_code, int(lst[2])])
            except IndexError:
                print lst
            element_code+=1
        if element_code==1:
...
def testCustomDayDialog():
    import gtk
    d = CustomDayDialog(None)
    gtk.main()
