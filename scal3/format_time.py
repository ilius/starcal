...
def compileTmFormat(fmt: str, hasTime: bool = True) -> CompiledTimeFormat:
	...
	while i < n:
		...
		if c1 == "d":
			...
			i += 2
			continue
		if c1 == "Q":  # calendar name (gregorian, jalali, ...)
			funcs.append(
				lambda _cell, calType, _tm: _(
		...
		if c1 == "a":
			funcs.append(
				lambda cell, _calType, _tm: core.weekDayNameAb[cell.weekDay],
			)
			pyFmt += "%s"
			i += 2
			continue
		if c1 == "A":
			funcs.append(
				lambda cell, _calType, _tm: core.weekDayName[cell.weekDay],
			)
			pyFmt += "%s"
			i += 2
			continue
		...
		if c1 == "D":  # %m/%d/%y
			...
			i += 2
			continue
		if c1 == "e":  # day of month, space padded; same as %_d
			funcs.append(
				lambda cell, calType, _tm: _(
					cell.dates[calType][2],
					fillZero=2,
		...
		if c1 == "F":  # %Y-%m-%d
			...
			i += 2
			continue
		if c1 == "g":  # not affected by calType!
			funcs.append(
				lambda cell, _calType, _tm: _(
		...
		if c1 == "G":  # not affected by calType!
			...
			i += 2
			continue
		if c1 == "V":  # not affected by calType!
			funcs.append(
				lambda cell, _calType, _tm: _(
					isow(cell.jd),
					fillZero=2,
		...
		if c1 == ":":
			...
			if c1 == "S":
				...
				i += 2
				continue
			if c1 == "s":  # seconds since 1970-01-01 00:00:00 UTC
				# funcs.append(lambda cell, calType, tm: _(
				# 	int(time.mktime(
				# 		a[2:7] + (
				# 			int(tm[2]),
			...
			if c1 == "T":  # %%H:%M:%S
				...
				i += 2
				continue
			if c1 == "X":  # locale's time representation (e.g., 23:13:48)
				funcs.append(
					lambda _cell, _calType, tm: (
						_(tm[0], fillZero=2)
						+ ":"
