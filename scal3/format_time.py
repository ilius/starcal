...
def compileTmFormat(format, hasTime=True) -> CompiledTimeFormat:
	...
	while i < n:
		...
		if c1 == "a":
			funcs.append(
				lambda cell, calType, tm: core.weekDayNameAb[cell.weekDay],
			)
			pyFmt += "%s"
			i += 2
			continue
		if c1 == "A":
			funcs.append(
				lambda cell, calType, tm: core.weekDayName[cell.weekDay],
			)
			pyFmt += "%s"
			i += 2
			continue
		...
		if c1 == ":":
			...
			if c1 == "r":  # %I:%M:%s PM
				funcs.append(
					lambda cell, calType, tm: _((tm[0] - 1) % 12 + 1, fillZero=2)
					+ ":"
					+ _(tm[1], fillZero=2)
					+ ":"
					+ _(tm[2], fillZero=2)
			...
			if c1 == "R":  # %H:%M
				funcs.append(
					lambda cell, calType, tm: _(tm[0], fillZero=2)
					+ ":"
					+ _(tm[1], fillZero=2),
				)
				pyFmt += "%s"
